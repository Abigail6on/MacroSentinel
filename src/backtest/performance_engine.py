...
# UPGRADE: Institutional Slippage (5 basis points per unit of turnover)
# If turnover is 1.0 (50% sold, 50% bought), the portfolio loses 0.05% to bid-ask spread
...
def get_rolling_optimal_weights(returns_window, assets):
    ...
    # 40% Cap Rule applied to the backtest to match the live optimizer
...
def calculate_backtest():
    print("--- Initializing Sentinel Pro Backtest Engine (Institutional) ---")
    ...
    last_regime = None
    last_weights = {}  # UPGRADE: Track previous weights for turnover calculation
    ...
    for i in range(len(df)):
        ...
        # --- UPGRADE: DYNAMIC TURNOVER & SLIPPAGE ---
        turnover = 0.0
        if last_weights: # If this is not the first day
            all_assets = set(list(final_weights.keys()) + list(last_weights.keys()))
            for ticker in all_assets:
                old_w = last_weights.get(ticker, 0.0)
                new_w = final_weights.get(ticker, 0.0)
                turnover += abs(new_w - old_w)
                
        # Calculate exactly how much the bid-ask spread cost us this hour
        ...
        # Deduct the exact transaction cost from the strategy's return
        hourly_ret -= transaction_cost
    ...
    print(f"[SUCCESS] Institutional Walk-Forward Optimization Complete.")
    print(f"          Final Alpha (Out-of-Sample, Net of Fees): {final_alpha:.2f}%")
