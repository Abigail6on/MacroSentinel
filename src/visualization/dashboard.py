...
def create_dashboard():
    if not os.path.exists(REGIME_DATA):
        print(f"[ERROR] No regime data found. Run regime_engine_v2.py first.")
        return
    ...
    # 2. Risk Matrix (Top Right) - FIXED with Colorbar & Current Marker
    scatter = axes[0, 1].scatter(df['Inflation_YoY'], df['Yield_Curve_10Y2Y'], 
                                 c=range(len(df)), cmap='Blues', s=100, edgecolors='black', alpha=0.6)
    
    # Adding the colorbar to show time progression
    cbar = fig.colorbar(scatter, ax=axes[0, 1])
    cbar.set_label('Time Progression (Hourly Steps)', rotation=270, labelpad=15)
    
    # Highlight the LATEST point in Red to show where we are NOW
    axes[0, 1].scatter(df['Inflation_YoY'].iloc[-1], df['Yield_Curve_10Y2Y'].iloc[-1], 
                       color='red', marker='*', s=200, edgecolors='black', label='Current Status', zorder=5)
    ...
    print(f"[SUCCESS] Dashboard generated with time-tracking: {OUTPUT_CHART}")
